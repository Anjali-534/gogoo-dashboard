...
function ComposeForm({ audience, onSent }: { audience: "drivers"|"riders"; onSent: () => void }) {
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Broadcast sent to ${meta.label}!`);
      setForm(emptyForm());
      onSent();
    } catch (e: any) {
  ...
  return (
    <form onSubmit={send} className="border border-gray-100 rounded-2xl p-5 bg-gray-50 mb-5">
      <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-4">New Broadcast</p>
    ...
    <div className="flex-1 min-w-0 bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">{active.length} active broadcast{active.length !== 1?"s":""}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="border-2 border-dashed border-gray-200 rounded-2xl py-12 text-center">
          ...
          <p className="text-xs text-gray-300 mt-1">Send your first broadcast above</p>
        </div>
      ) : (
        <>
          {active.length > 0 && (
