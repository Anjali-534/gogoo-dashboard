...
type Driver = {
  ...
  vehicle_type: string; vehicle_category: string; vehicle_number: string;
...
const CATEGORY_COLOR: Record<string, string> = { truck: "#3B82F6", cab: "#FF6B2B", ambulance: "#EF4444" };
const CATEGORY_EMOJI: Record<string, string> = { truck: "🚛", cab: "🚗", ambulance: "🚑" };
...
export default function MapComponent() {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.addLayer({
        ...
        paint: {
          "circle-color": ["get", "color"], "circle-radius": 14,
          "circle-stroke-width": 3, "circle-stroke-color": "#fff",
        },
      });
      map.addLayer({
  ...
  // ── Fetch drivers + bookings ─────────────────────────────────────────────
  const fetchData = async () => {
    try {
      const headers = { Authorization: `Bearer ${token()}` };
      const [dRes, bRes] = await Promise.all([
        axios.get(`${API}/gogoo/live/drivers`, { headers }),
        axios.get(`${API}/gogoo/live/bookings`, { headers }),
      ]);
  ...
  // ── Update driver markers (clustered, animated, stale-aware) ────────────
  useEffect(() => {
    ...
    const visible = drivers.filter(d =>
      ...
      (catFilter === "all" || d.vehicle_category === catFilter)
    );
    ...
    visible.forEach(d => {
      ...
      const color = stale ? STALE_COLOR : (CATEGORY_COLOR[d.vehicle_category] || "#FF6B2B");
      const emoji = CATEGORY_EMOJI[d.vehicle_category] || "🚗";
      ...
      const popupHtml = `
        <div style="font-family:system-ui;min-width:170px;padding:4px">
          <div style="font-weight:800;font-size:14px;margin-bottom:4px">${emoji} ${d.name}</div>
      ...
      propsById[d.id] = { id: d.id, color, emoji, popupHtml };
    });
    ...
    const step = (now2: number) => {
      ...
      if (t < 1) {
        ...
        driverPrevCoords.current = toCoords;
      }
    };
    animFrameRef.current = requestAnimationFrame(step);
  }, [drivers, bookings, catFilter, mapLoaded]);

  // ── Update booking pickup markers ────────────────────────────────────────
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapLoaded) return;
    ...
    const visible = bookings.filter(b => catFilter === "all" || b.category === catFilter);
        ...
        properties: {
          popupHtml: `
            <div style="font-family:system-ui;min-width:170px;padding:4px">
              ...
              <div style="color:#6B7280;font-size:12px">${b.service_name || b.category} · ${b.status.replace(/_/g, " ")}</div>
  ...
  // /gogoo/live/drivers only returns online drivers with a GPS fix, so
  // everything here is "online" by construction — no offline bucket to show.
  const online     = drivers.filter(d => catFilter === "all" || d.vehicle_category === catFilter);
  const idle       = online.filter(d => !d.active_booking_id);
  const searching  = bookings.filter(b => b.status === "searching" && (catFilter === "all" || b.category === catFilter));
  const activeRide = bookings.filter(b => b.status !== "searching" && (catFilter === "all" || b.category === catFilter));
            ...
            {online.length === 0 ? (
              ...
              <div key={d.id} onClick={() => panTo(d)}
                ...
                <p className="text-xs mt-1 ml-12">
                  {d.active_booking_id
                    ? <span className="text-orange-500">🚕 On a ride</span>
